...
for l in ll:
    ...
    if old in all_printk:
        if old in printk_like:
            old_level = get_printk_level(old_content)
        if old in debug_printk:
            old_level = 7
        if old in err_printk:
            old_level = 3
        if old in notice_printk:
            old_level = 5
        if old in warn_printk:
            old_level = 4
        if old in cont_printk:
            old_level = 8
    else:
        old_level = get_level(old)
   
    if new in all_printk:
        if new in printk_like:
            new_level = get_printk_level(new_content)
        if new in debug_printk:
            new_level = 7
        if new in err_printk:
            new_level = 3
        if new in notice_printk:
            new_level = 5
        if new in warn_printk:
            new_level = 4
        if new in cont_printk:
            new_level = 8
    else:
        new_level = get_level(new)
    
    print(str(old_level) + "->" + str(new_level))



# python script2.py > out
